...
public class TrigramFeatureTransformer extends FeatureTransformer {
    ...
    protected List<String> toFeature(List<String> words, List<Pos> tags, int i) {
        ...
        features.add(String.format("x[0:1]=%s-%s", words.get(i), words.get(i + 1)));
        features.add(String.format("x[0:1]=%s-%s, y[1]=%s", words.get(i), words.get(i + 1),
                tags.get(i + 1)));
